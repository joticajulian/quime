...
function removeAlreadyInDB(records, db){
...
async function processNewFiles(db) {
  ...
  for(var f in csv_filenames){
    ...
    var result = removeAlreadyInDB(records, db)
  ...
  return {changed_from, db}
}

function insertRecord(_record, id, db){
...
function recalculateBalances(index, state){
...
async function main() {
  ...
  var db = fs.readFileSync(config.DB_FILENAME, 'utf-8')
  var state = fs.readFileSync(config.STATE_FILENAME, 'utf-8')

  db = JSON.parse(db)
  state = JSON.parse(state)

  var result = await processNewFiles(db)
  var changed_from = result.changed_from
  db = result.db
  state = recalculateBalances(0, state)
